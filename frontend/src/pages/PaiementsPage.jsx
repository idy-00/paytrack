...
import { Search, Download, Wallet, Calendar, Receipt, TrendingUp, CreditCard } from 'lucide-react'
import { MOCK_SALES, formatAmount, formatDate } from '@/lib/mockData'
...
/* ── Données ── */
const ALL_PAYMENTS = MOCK_SALES.flatMap(sale =>
  sale.payments.map(p => ({
    ...p,
...
const METHOD_STYLES = {
  ...
  virement:     { bg: '#F3F4F6',              color: '#374151' },
...
export default function PaiementsPage() {
  ...
  const totalEncaisse = ALL_PAYMENTS.reduce((acc, p) => acc + p.amount, 0)
  const totalCeMois   = ALL_PAYMENTS
    .filter(p => (p.date || '').startsWith('2026-07'))
    .reduce((acc, p) => acc + p.amount, 0)
  const avgAmount = ALL_PAYMENTS.length
    ? Math.round(totalEncaisse / ALL_PAYMENTS.length)
    : 0
  ...
  const filtered = ALL_PAYMENTS.filter(p => {
    ...
    const matchSearch = !q ||
      ...
      p.sale_reference.toLowerCase().includes(q) ||
      (p.receipt_no || '').toLowerCase().includes(q) ||
      p.article.toLowerCase().includes(q)
    const matchMethod = methodFilter === 'Tous' ||
      (p.payment_method || 'especes').replace('_', ' ').toLowerCase() === methodFilter.toLowerCase()
    return matchSearch && matchMethod
  })

  const filteredTotal = filtered.reduce((acc, p) => acc + p.amount, 0)
    ...
    <div className="max-w-6xl mx-auto space-y-5 pb-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{ALL_PAYMENTS.length} paiements enregistrés</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { icon: Calendar,  bg: '#DCFCE7', color: SUCCESS, label: 'Ce mois (juil.)',      value: formatAmount(totalCeMois)   },
          { icon: Receipt,   bg: '#F3F4F6', color: '#374151', label: 'Nb de paiements',   value: ALL_PAYMENTS.length         },
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="search"
            placeholder="Chercher par client, référence, reçu, article…"
            value={search}
        ...
        <div className="flex gap-2 flex-wrap" role="group" aria-label="Filtrer par mode">
      ...
      <div className="card hidden md:block overflow-x-auto">
        <table className="w-full" role="table" aria-label="Liste des paiements">
          <thead>
            <tr>
              {['Reçu', 'Date', 'Client', 'Article', 'Référence vente', 'Mode', 'Type', 'Montant'].map(h => (
              ...
              <tr key={p.id || i} className="table-row">
                ...
                <td className="table-cell">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold ${
                    p.type === 'acompte' ? 'bg-blue-50 text-blue-700' :
                    p.type === 'solde'   ? 'bg-green-50 text-green-700' :
                    'bg-gray-100 text-gray-600'
                  }`}>
                    {p.type === 'acompte' ? 'Acompte' : p.type === 'solde' ? 'Solde' : 'Tranche'}
                  </span>
                </td>
                <td className="table-cell text-right">
                  <span className="amount text-sm font-bold" style={{ color: SUCCESS }}>
          ...
          {filtered.length > 0 && (
            <tfoot>
              <tr className="border-t-2 border-gray-200 bg-gray-50">
                <td colSpan={7} className="px-5 py-3 text-xs font-semibold text-gray-500">
                  Total ({filtered.length} paiement{filtered.length > 1 ? 's' : ''})
                </td>
                <td className="px-5 py-3 text-right">
                  <span className="amount text-sm font-bold text-gray-900">
          ...
          <div key={p.id || i} className="card p-4">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="text-right flex-shrink-0">
                <p className="amount text-sm font-bold" style={{ color: SUCCESS }}>
                  {formatAmount(p.amount)}
                </p>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-100">
              ...
              <span className={`text-xs font-medium ${
                p.type === 'acompte' ? 'text-blue-600' :
                p.type === 'solde'   ? 'text-green-600' : 'text-gray-500'
              }`}>
                {p.type === 'acompte' ? 'Acompte' : p.type === 'solde' ? 'Solde' : 'Tranche'}
              </span>
            </div>
