...
import { Plus, Search, Phone, Mail, MapPin, ArrowRight, Users } from 'lucide-react'
import { MOCK_CLIENTS, MOCK_SALES, formatDate } from '@/lib/mockData'
    ...
    <div className="max-w-6xl mx-auto space-y-5 pb-8">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{MOCK_CLIENTS.length} clients enregistrés</p>
        </div>
      ...
      <div className="card hidden md:block overflow-x-auto">
        <table className="w-full" role="table" aria-label="Liste des clients">
          <thead>
            <tr>
              {['Client', 'Téléphone', 'Email', 'Ville', 'Client depuis', 'Ventes actives', ''].map(h => (
          ...
          <tbody>
            {filtered.map(client => {
              const activeSales = MOCK_SALES.filter(s => s.client.id === client.id && s.status !== 'solde')
              return (
                <tr key={client.id} className="table-row">
                  ...
                  <td className="table-cell">
                    <button className="btn btn-ghost btn-icon btn-sm" aria-label="Voir le client">
                      <ArrowRight size={14} />
                    </button>
                  </td>
                </tr>
