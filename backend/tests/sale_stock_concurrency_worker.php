...
while (DB::table('audit_concurrency_barrier')->where('barrier_id', $barrierId)->count() < 2) {
    if (microtime(true) >= $deadline) {
        throw new RuntimeException('Second concurrent sale worker did not reach the barrier.');
    }
    usleep(10_000);
}

$request = Request::create('/api/sales', 'POST', $payload, [], [], [
    'HTTP_AUTHORIZATION' => 'Bearer ' . getenv('PAYTRACK_AUDIT_TOKEN'),
